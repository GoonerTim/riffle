...
(включая ISO-8601 timestamps); вложенные объекты разворачиваются; типы авто-расширяются за
пределами сэмпла; поддержаны override `--schema`, проекция колонок (`--select`/`--exclude`/
`--rename`), прозрачный вход gzip/zstd и многопоточная конвертация (`--threads`, детерминированный
вывод). Известные ограничения: при `--threads > 1` схема фиксируется из сэмпла (без межбатчевого
widening); вложенные данные разворачиваются в плоские колонки, а не в нативные Parquet-struct.
