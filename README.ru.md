...
Riffle **не** лидер по скорости. DuckDB (~360–595 МБ/с) и PyArrow (~280–390 МБ/с) быстрее;
Riffle держит **~80 МБ/с**, опережая pandas (~40 МБ/с). Парсинг уже использует **on-demand**
API simdjson, а append в Arrow **батчевые** по колонкам; оставшаяся стоимость — это
материализация строк на каждую запись (аллокации для развёрнутых путей/значений и копии
буфера на строку), а не парсинг JSON или append в Arrow. Кодек сжатия почти не влияет.
