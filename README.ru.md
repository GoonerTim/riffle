...
(библиотека + CLI), написана test-first (~68 тестов). C++23. Схема выводится автоматически
(включая ISO-8601 timestamps); вложенные объекты разворачиваются. Известные ограничения:
override `--schema` доступен только через библиотеку, расширение типов — только int→double.
