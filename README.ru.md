...
- **Быстро на одном ядре** — SIMD-парсинг JSON
  ...
  держат **~100 МБ/с** входа на одном ядре.
...
Riffle **не** лидер по скорости. DuckDB (~350–595 МБ/с) и PyArrow (~280–405 МБ/с) быстрее;
Riffle держит **~100 МБ/с**, опережая pandas (~40 МБ/с). Парсинг использует **on-demand** API
simdjson, поля пишутся **напрямую в построители колонок** (без промежуточного объекта-строки
и без аллокации пути на каждое поле), а append в Arrow **батчевые** по колонкам. Оставшаяся
стоимость — построение Arrow-массивов и чтение строк; кодек сжатия почти не влияет.
