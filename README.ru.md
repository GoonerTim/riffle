...
pandas — до **4.2 ГБ на входе 359 МБ (~×12)**, pyarrow — ~810 МБ, duckdb — ~490 МБ и растёт с
...
поднимается до **~180–220 МБ** (несколько чанков «в полёте» на воркеров), но память **по-прежнему
...
достигает **~360–380 МБ/с** — **самый быстрый** из всех на этом железе, **ценой памяти** (~180–220 МБ
вместо ~80 МБ — всё ещё ограниченной и сильно ниже pyarrow/pandas). То есть выбираешь компромисс:
минимальная память в один поток или максимальная скорость с умеренным, ограниченным ростом памяти.

### Масштабирование по `--threads`

![Скорость от числа потоков](docs/img/bench_threads.png)

Скорость растёт с ядрами: от ~95 МБ/с на 1 потоке до **~380 МБ/с на 8** (датасет 120 МБ). Вывод
**byte-identical и детерминированный** при любом числе потоков (воркеры парсят+строят чанки; единый
писатель пишет батчи в порядке входа). Цена — память: каждый чанк «в полёте» добавляет ограниченную
величину, поэтому пик RSS умеренно растёт с `--threads`, но никогда — с размером входа.
